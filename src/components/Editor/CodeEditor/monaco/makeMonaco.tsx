...
import { editor, Uri, KeyMod, KeyCode, IKeyboardEvent, Range, IRange, IScrollEvent } from "monaco-editor/esm/vs/editor/editor.api"
import { splitFileName } from "@lincode/utils"
import { useResizeObserver } from "@lincode/hooks"

type SuggestionChangeEvent = { show: boolean, suggestions: Array<string>, focused: string }

const noSuggestion: SuggestionChangeEvent = { show: false, suggestions: [], focused: "" }
...
export default () => {
    ...
    const Monaco: React.FC<{
        ...
        useEffect(() => {
            ...
            editorInstance.addCommand(KeyMod.CtrlCmd | KeyMod.Alt | KeyCode.KEY_S, currentSaveAll = () => {
                const entries: Array<[string, string]> = []
                if (filesRef.current)
                    for (const key of Object.keys(filesRef.current)) {
                        const model = editor.getModel(Uri.parse(key))
                        model && entries.push([model.uri.path, model.getValue()])
                    }
                return onSaveAll?.(entries)
            })

            editorInstance.addCommand(KeyMod.CtrlCmd | KeyCode.Enter, () => {
                editorInstance.trigger("monaco", "editor.action.insertLineAfter", undefined)
            })
            ...
            editorInstance.addCommand(KeyMod.CtrlCmd | KeyMod.Shift | KeyCode.KEY_Z, () => {
                currentOnRedo?.()
                editorInstance.trigger("monaco", "redo", undefined)
            })
            ...
            onCursorPositionChange && editorInstance.onDidChangeCursorPosition(onCursorPositionChange)
            onScrollChange && editorInstance.onDidScrollChange(onScrollChange)

            onChange && editorInstance.onDidChangeModelContent(e => {
                ...
                editorInstance.onDidFocusEditorText(() => {
                    ...
                    observer = new MutationObserver(() => {
                        ...
                        if (widgetEl.classList.contains("visible")) {
                            ...
                            const rows = widgetEl.querySelector(".monaco-list-rows")
                            if (rows)
                                for (const item of rows.children) {
                                    const suggestion = item.textContent?.trim()
                                    if (suggestion) {
                                        suggestions.push(suggestion)
                                        item.classList.contains("focused") && (focused = suggestion)
                                    }
                                }
                            suggestions.length && (suggestionNew = { suggestions, focused, show: true })
                        }
                        suggestionOld !== suggestionNew && (onSuggestionChange(suggestionNew))
        ...
        const [viewStateMap] = useState(() => new Map<string, editor.ICodeEditorViewState | undefined | null>())
        ...
        useEffect(() => {
            ...
            for (const [key, value] of Object.entries(files)) {
                ...
                const newModel = editor.createModel("", languages?.[extension ?? ""] ?? language, uri)
        ...
        useEffect(() => {
            ...
            fileRef.current && viewStateMap.set(fileRef.current, editorInstance.saveViewState())
    ...
    const setValue = (val: string) => void currentEditor?.executeEdits("monaco", [{
        range: new Range(1, 1, Infinity, Infinity), text: val
    }])
    ...
    const highlightLine = (line: number) => {
        ...
        decorationMap.set(currentModel, currentModel.deltaDecorations([], [{
            range: new Range(line, 1, line, 1),
            options: { className: "monaco-highlight", isWholeLine: true }
        }]))
    ...
    const controls = {
        ...
        clearHighlight: () => currentModel?.deltaDecorations(decorationMap.get(currentModel) ?? [], []),

        replace: (lineStart: number, columnStart: number, lineEnd: number, columnEnd: number, text: string) => {
            currentModel?.pushStackElement()
            currentEditor?.executeEdits("monaco", [{
                range: new Range(lineStart, columnStart, lineEnd, columnEnd), text
            }])
        },
        ...
        insertLine: (line: number, text: string) => {
            ...
            currentEditor?.executeEdits("monaco", [{
                range: new Range(line, col + 1, line, col + 1), text: "\n" + text, forceMoveMarkers: true
            }])
        },
        ...
        setSelection: (selection: IRange) => void currentEditor?.setSelection(selection),
        ...
        save: () => {
            currentModel && currentOnSave?.(currentModel.getValue(), currentModel.uri.path)
            currentEditor?.focus()
        },
        ...
        suggest: () => void currentEditor?.trigger("monaco", "editor.action.triggerSuggest", undefined),

        selectNextSuggestion: () => void currentEditor?.trigger("monaco", "selectNextSuggestion", undefined),

        focused: () => currentFocused,

        jumpTo: (uri: string, line: number, fileOpen: (uri: string) => void) => {
            fileOpen(uri)
